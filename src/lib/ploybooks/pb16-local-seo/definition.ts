      ...
      async run(ctx) {
        ...
        const draftRaw = await llm.generateStructured({
          system:
            ...
            "Houston, 5-year warranty, in-house survey/permit/fabricate/install — awnings and " +
            "canopies are fabricated in-house too). AWNING/CANOPY pages: served STATEWIDE (no " +
            "Houston-radius framing), and do NOT claim the 5-year warranty or any price/range " +
            "for them — neither is confirmed for that product line. HARD RULES: " +
